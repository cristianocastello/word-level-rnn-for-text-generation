...
def train():
    ...
    try:
        model.load_weights("<path to your text saved weights>")
    ...
    for iteration in range(1, 750):
        ...
        with open('<path to where you want to save your model architecture>', 'w') as f:
            f.write(json_string)
        model.save_weights('<path to where you want to save your weights>', overwrite=True)
...
def generate_from_word_level_rnn(maxlen=30, diversity=1.0, min_sent_len=10, max_sent_len=65):
    ...
    response = ""
    model = model_from_json(open('<path to your text file>').read())
